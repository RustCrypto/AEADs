...
//! mode of operation ([RFC 5297][1]). The interface is based on the [Rogway paper][2].
//!
//! # Deterministic Authenticated Encryption Example
//! Deterministic encryption with aditional data. Suitable for example Key-Wrap.
//! Based on the test vector in [RFC 5297 Apendix: A1][3]
...
//! Nonce based encryption with multiple aditional data vectors.
//! Based on the test vector in [RFC 5297 Apendix: A2][4]
#![cfg_attr(feature = "std", doc = "```")]
#![cfg_attr(not(feature = "std"), doc = "```ignore")]
//! # fn main() -> Result<(), Box<dyn std::error::Error>> {
//! use aes_siv::{siv::Aes128Siv, KeyInit};
    ...
    M: Mac<OutputSize = U16> + FixedOutputReset + KeyInit,
{
    ...
    /// Or returning an error in the even the provided authentication tag does not match the given ciphertext.
    #[cfg(feature = "alloc")]
