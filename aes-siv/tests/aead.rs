    ...
    ($aead:ty, $vectors:expr) => {
        ...
        fn encrypt_inout_detached() {
            for vector in $vectors {
                let key = Array(*vector.key);
                let nonce = Array(*vector.nonce);
                let buffer = vector.plaintext.to_vec();
                let mut buffer = aead::dev::MockBuffer::from(buffer);

                let cipher = <$aead>::new(&key);
                let tag = cipher
                    .encrypt_inout_detached(&nonce, vector.aad, buffer.to_in_out_buf())
                    .unwrap();
                let (expected_tag, expected_ciphertext) = vector.ciphertext.split_at(16);
                assert_eq!(expected_tag, &tag[..]);
                assert_eq!(expected_ciphertext, buffer.as_ref());
            }
        }
        ...
        fn decrypt_inout_detached() {
            for vector in $vectors {
                let key = Array(*vector.key);
                let nonce = Array(*vector.nonce);
                let tag = Array::try_from(&vector.ciphertext[..16]).unwrap();
                let buffer = vector.ciphertext[16..].to_vec();
                let mut buffer = aead::dev::MockBuffer::from(buffer);

                <$aead>::new(&key)
                    .decrypt_inout_detached(&nonce, vector.aad, buffer.to_in_out_buf(), &tag)
                    .unwrap();

                assert_eq!(vector.plaintext, buffer.as_ref());
            }
        }
